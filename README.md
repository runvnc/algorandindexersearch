...
// When displaying results, format as ALGOs! (divide by 1000000)
// (Same for asset transfer amounts, divide by 1000000!)
...
async function examples() {
  ...
  let afterTime = new Date('02-16-23').toISOString() // OPTIONAL, OMIT UNLESS SPECIFIED
  let limit = 1 // used for example only. OMIT THIS IN NORMAL USAGE, defaults to 1000
  ...
  // Note: can optionally combine some or combine all parameters, such as txType, assetID, address!!
  ...
  await logSearch('pay', {txType:'pay', limit, address, addressRole: 'sender'})
  ...
  await logSearch('acfg', {txType:'acfg'})
